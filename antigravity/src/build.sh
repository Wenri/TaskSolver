#!/usr/bin/env bash
# Build antigravity.so without `make` (this host has no make). Equivalent to the Makefile.
set -euo pipefail
...
${CC:-gcc} -O2 -g -fPIC -Wall -Wextra -std=gnu11 \
    ...
    -I../vendor/frida-gum $PYINC $UAPI \
    -shared -Wl,--exclude-libs,ALL -Wl,--gc-sections \
    -o ../build/antigravity.so antigravity.c pybridge.c \
    ../vendor/frida-gum/libfrida-gum.a $PYLD -lrt -lresolv -ldl -lm -pthread

echo "built ../build/antigravity.so ($(stat -c%s ../build/antigravity.so) bytes)"
